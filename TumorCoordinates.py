...
"""CROPS TUMOR"""
hah = CT.GetImage(10)
x1 = cfgt['x1'] #Exact tumor boundaries
x2 = cfgt['x2']
y1 = cfgt['y1']
y2 = cfgt['y2']
z1 = cfgt['z1']
...
with tf.Session() as sess:
    ...
    for i in tqdm(range(590)):
        ...
        np.save(path + 'outputsNEWall/{}.npy'.format(i), output)

#np.save('c:/balbla.npy', convoluted)
arr=[]
for i in tqdm(range(550)): #only 550 scans were used***
    arr.append(CT.GetMaxWeightedIndex(i,path + 'outputsNEWall/{}.npy'))
np.save(path + 'XYZ/vsiXYZ.npy', arr=arr) #SAVES tumor coordinates as array
